...
def main(command_line=None):
    ...
    add.add_argument(
        "-n", "--num", action="store", type=int, required=True, help="The plane's numer"
    )
    add.add_argument(
        "-t", "--typ", action="store", required=True, help="The plane's type"
    )

    # Создать субпарсер для отображения всех работников.
    _ = subparsers.add_parser(
        "display", parents=[file_parser], help="Display all planes"
    )

    # Создать субпарсер для выбора работников.
    select = subparsers.add_parser(
        "select", parents=[file_parser], help="Select the planes"
    )
    select.add_argument(
        "-T", "--type", action="store", required=True, help="The required type"
    )
